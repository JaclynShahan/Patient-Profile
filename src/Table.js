...
class Table extends Component {
  ...
  componentDidMount = () => {
    Axios.get(`/api/getCharges`).then(resp => {
      console.log(resp)
  ...
  onDelete = i => {
    Axios.delete(`/api/deleteCharge/${i}`).then(resp => {
      console.log(resp)
      this.setState({ charges: resp.data })
    })
  }
  ...
  render () {
    const tableRows = this.props.users.map((person, indexPoint) => {
      return (
        <tr key={indexPoint}>
          <Popconfirm
            onConfirm={() => this.props.onDelete(person.id)}
            title='Are you sure?'
          >
            <td>
              <Button>
                <Icon className='iconTrash' type='delete' />
              </Button>
            </td>
          </Popconfirm>
